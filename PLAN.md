...
All six phases merged to main via the local gate, coverage ≥85%, `uv build` yields valid wheel+sdist (`twine check` passes), wheel installed into a clean venv runs `khatt "مرحبا"` with correct connected-script output, `app.py` runs locally, CI workflows committed and ready for first GitHub push, tag `v0.1.0`.

## Open questions

1. **Tashkeel default:** I propose **strip by default** (diacritics are noise at grid resolution) with `--keep-tashkeel` / `tashkeel=True` to render them. OK, or prefer keep-by-default?
2. **PyPI name:** `khatt` availability on PyPI is unverified (no network check done in planning). If taken at release time: fallback candidates `khatt-art`, `pykhatt` — your call then; doesn't block PRs 1–5.
3. **Fourth font:** Rubik chosen for strong Latin+Arabic dual coverage (good for mixed strings). Swap for something else (e.g. Lateef, Reem Kufi) if you'd rather.
4. **Terminal cell aspect:** default 0.5 (cell twice as tall as wide) — standard assumption, exposed as an internal constant, not a public API param in v0.1.0.
